...
import { getBlogPost, updateBlogPost, type BlogPost, OrganizationMember, getOrganizationMembers, getUserOrganization } from "@/lib/firebase/firestore"
...
export default function EditBlogPostPage() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const finalExcerpt = strippedContent.substring(0, 150) + "..."
            await updateBlogPost(post.id, {
                title,
                content,
                excerpt: finalExcerpt, // Update excerpt based on new content
    ...
    if (loading) {
        return <div className="flex h-screen items-center justify-center text-muted-foreground bg-background">Loading editor...</div>
    }

    if (!post) {
        return <div className="flex h-screen items-center justify-center text-muted-foreground bg-background">Post not found.</div>
    }
            ...
            <header className="sticky top-0 z-50 w-full bg-background/80 backdrop-blur-md border-b">
                <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <span className="text-sm text-muted-foreground font-medium hidden md:inline-block">Editing</span>
                        {orgId && (
                            <div className="ml-2">
                                <UserGroupSelect
                                    orgId={orgId}
